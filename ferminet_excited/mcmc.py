...
def make_mcmc_steps(batch_networks,
                   batch_per_device,
                   steps=10,
                   atoms=None,
                   ndim=3,
  ...
  @jax.jit
  def mcmc_step(params, datas, key, widths):
    """Performs a set of MCMC steps.
    ...
    Returns:
      ...
      updated RNG state and pmove the average probability a move was accepted.
    """
    def step_func(batch_network, inner_fun, width, atoms, ndim, blocks, param):
        def step_fn(i, x):
          return inner_fun(
              param,
              *x,
    ...
    new_datas = []
    pmoves = []
    for data, param, i in zip(datas, params, range(len(datas))):
      pos = data.positions
      nsteps = steps * blocks
      logprob = 2.0 * batch_networks[i](
          param, pos, data.spins, data.atoms, data.charges
      )
      step_fns = step_func(batch_networks[i], inner_fun, widths[i], atoms, ndim, blocks, param)
      new_data, key, _, num_accepts = lax.fori_loop(
          0, nsteps, step_fns, (data, key, logprob, 0.0)
      )
      pmove = jnp.sum(num_accepts) / (nsteps * batch_per_device)
      pmove = constants.pmean(pmove)
      new_datas.append(new_data)
      pmoves.append(pmove)
    return new_datas, pmoves
